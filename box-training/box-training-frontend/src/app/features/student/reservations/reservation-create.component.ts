...
export class ReservationCreateComponent implements OnInit {
  ...
  protected readonly filteredSchedules = computed(() => {
    ...
    if (!planType || schedules.length === 0) {
      return [];
    }

    const filtered = schedules.filter((schedule) => {
      console.log(
        `Comparing ${schedule.classType} === ${planType}:`,
        schedule.classType === planType
      );
      return schedule.classType === planType;
    });
  ...
  protected getScheduleDisplayText(scheduleId: string): string {
    ...
    return schedule
      ? this.schedulesService.formatScheduleDisplay(schedule)
      : '';
  }
