  ...
  updatePlan(updateData: UpdatePlanDto): Observable<ApiResponse<Plan>> {
    return of(null).pipe(
      ...
      map(() => {
        ...
        const hasActiveStudents = studentPlans.some(sp =>
          sp.planId === updateData.id && sp.status === StudentPlanStatus.ACTIVE
        );

        // Si hay alumnos activos, no permitir cambio de tipo
        if (hasActiveStudents && updateData.type && updateData.type !== existingPlan.type) {
  ...
  activateStudentPlan(activateData: ActivatePlanDto): Observable<ApiResponse<StudentPlan>> {
    return of(null).pipe(
      ...
      map(() => {
        ...
        const studentPlans = this.mockDataService.getStudentPlans();
        const previousPlan = studentPlans.find(sp =>
          sp.studentId === activateData.studentId && sp.status === StudentPlanStatus.ACTIVE
        );
    ...
    return of(null).pipe(
      ...
      map(() => {
        ...
        const daysFreezing = Math.ceil(
          (freezeData.endDate.getTime() - freezeData.startDate.getTime()) /
          (1000 * 60 * 60 * 24)
        );
        ...
        // Actualizar plan
        const updatedPlan = this.mockDataService.updateStudentPlan(
          freezeData.studentPlanId,
          {
  ...
  getStatisticsPlans(): Observable<any> {
    return of(null).pipe(
      ...
      map(() => {
        ...
        const activeAssignments = studentPlans.filter(sp =>
          sp.status === StudentPlanStatus.ACTIVE
        ).length;
